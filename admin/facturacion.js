...
document.addEventListener("DOMContentLoaded",function(){
  ...
  async function createInvoiceFromOrder(){
    ...
    try{
      ...
      const stored=await getStoredDoc(createdId,created);await notifyInvoice(stored||created);setMsg(`Factura ${created.numero} generada correctamente.`);e.pedido.value="";renderPreview();setTimeout(()=>downloadPdf(stored||{...created,id:createdId}),100);
  ...
  async function getStoredDoc(id,fallback){
    if(!id)return fallback||null;
    try{
      const snap=await C.doc(id).get({source:"server"});
      if(snap.exists)return {id:snap.id,...snap.data()};
    }catch(err){
      console.warn("No se pudo releer el documento emitido desde Firestore:",err);
    }
