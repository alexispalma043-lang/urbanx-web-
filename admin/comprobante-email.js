...
(function(){
  ...
  function params(item){
    const to=text(item?.comprador?.email).toLowerCase();
    const name=text(item?.comprador?.razonSocial||"Cliente SIXTEEN");
    const type=typeName(item);
    const number=text(item?.numero||item?.id)||"—";
    const total=money(item?.totales?.importeTotal??item?.totales?.total);
    const base=money(item?.totales?.baseImponible??item?.totales?.subtotalSinImpuestos??item?.totales?.subtotal);
    const tax=money(item?.totales?.impuesto??item?.totales?.iva);
    const link=item?.clienteUid?accountUrl():"";
    const due=text(item?.fechaVencimiento);
    return {
      to_email:to,to_name:name,subject:`${type} ${number}`,
      document_type:type,document_number:number,document_total:total,
      message:[
        `SIXTEEN emitió tu ${type.toLowerCase()} ${number}.`,
        item?.tipoDocumento==="FACTURA"?`Base: ${base}\nIVA: ${tax}\nTotal: ${total}.`:`Total: ${total}.`,
        due?`Fecha de vencimiento: ${due}.`:"",
        link?`Puedes consultar y descargar el PDF desde Mi Cuenta:\n${link}`:"Puedes solicitar el PDF al equipo de SIXTEEN o responder a este correo.",
        `Este documento es comercial y fue generado por el facturador propio de SIXTEEN; no representa autorización tributaria del SRI.`
      ].filter(Boolean).join("\n\n")
    };
  }
